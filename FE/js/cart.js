...
// Lấy tất cả các phần tử có class 'product_price'
    ...
    for (let i = 0; i < productPrices.length; i++) {
        ...
        const price = parseFloat(priceText.replace('đ', '').replace('.', '').replace(',', '.'));
        // Lấy số lượng từ phần tử input có class 'quant'
        const quantity = parseInt(productPrices[i].parentNode.querySelector('.quantity').value);
    ...
    document.getElementById('total_price').innerText = totalPrice.toLocaleString() + ' đ';
}
...
function checkEmptyCart() {
    ...
    if (cartItems.length === 0) { // Nếu danh sách rỗng
        document.querySelector('.order_checkout').innerHTML = '<p>Giỏ hàng rỗng!</p>'; // Thay đổi nội dung của phần tử có class "order_checkout"
    }
}